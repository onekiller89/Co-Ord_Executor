...
Before cutover, re-fetch GitHub and re-check both worktrees because the live bot may create more extraction commits. Review the 67 extraction records before merging. One confirmed source/content mismatch is `extractions/2026-03-04_agent-skills-for-business-explained-in-less-than-15-minutes.md`: YouTube oEmbed currently identifies its URL as “How I Turn Any Business Task Into a Reusable AI Skill” by Sean Kochel, while its summary, actions and link description describe a Tetris tutorial. `INDEX.md` carries that mismatched Game Development category. Preserve the original as evidence until the source is re-extracted and checked. The other records have not been individually validated. Run a representative end-to-end extraction after restart.
