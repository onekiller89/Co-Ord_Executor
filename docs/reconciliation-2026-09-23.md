...
Before cutover, re-fetch GitHub and re-check both worktrees because the live bot may create more extraction commits. Review the 67 extraction records before merging. One confirmed internal contradiction is `extractions/2026-03-04_agent-skills-for-business-explained-in-less-than-15-minutes.md`: its title says Agent Skills, while its summary, actions and link description describe a Tetris tutorial. `INDEX.md` carries that mismatched category. Preserve the original as evidence until the source is re-extracted and checked. The other records have not been individually validated. Run a representative end-to-end extraction after restart.
